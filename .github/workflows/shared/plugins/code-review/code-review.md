...
- Ignore generated artifacts and repo metadata by default, especially `.lock.yml` files and `.gitattributes`, unless the PR also changes the source-of-truth files in a way that makes those generated or metadata changes materially important to behavior.
...
- Differences that exist only in generated `.lock.yml` files or `.gitattributes` metadata, unless they expose a real issue in the source workflow or review prompt
